...
export default function TesLanjutan() {
            ...
            <div className="min-h-screen bg-white">
                <div className="max-w-4xl mx-auto px-6 py-20 flex flex-col items-center text-center space-y-12">
                    ...
                    <div className="space-y-6">
                        <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 tracking-tight">Tes Lanjutan (Analisis Rapor)</h1>
                        ...
                        <div className="bg-orange-50 p-6 rounded-2xl border border-orange-100 flex items-start gap-4 text-left max-w-xl mx-auto">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
                        <button onClick={() => navigate(-1)} className="px-8 py-4 rounded-2xl font-bold text-gray-500 hover:text-blue-600 transition-all flex items-center justify-center gap-2">
            ...
            <div className="min-h-screen bg-gray-50/50 pb-32">
                <nav className="fixed top-0 left-0 right-0 bg-white/80 backdrop-blur-md border-b border-gray-100 z-50">
                    <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
                        <button onClick={() => setStep('intro')} className="text-gray-500 hover:text-blue-700 flex items-center gap-1 font-bold text-sm">
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-gray-900 text-white">
                                        <th className="p-5 text-sm font-bold border-r border-gray-800">Mata Pelajaran</th>
                                        {SEMESTERS.map(sem => (
                                            <th key={sem} className="p-5 text-sm font-bold text-center border-r border-gray-800 last:border-0">
                                                Sem {sem}
                                            </th>
                                        ))}
                                    </tr>
                                ...
                                <tbody className="text-gray-600">
                                    {SUBJECT_LIST.map((sub, idx) => (
                                        <tr key={sub} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                                            <td className="p-4 font-bold text-gray-900 border-r border-gray-100">{sub}</td>
                                            {SEMESTERS.map(sem => (
                                                <td key={sem} className="p-2 border-r border-gray-100 last:border-0">
                                                    <input
                                                        ...
                                                        className="w-full p-3 text-center rounded-xl bg-transparent focus:bg-white focus:ring-2 focus:ring-blue-600 focus:shadow-lg outline-none transition-all font-bold text-blue-700"
                                                    />
                                                </td>
                                            ))}
                                        </tr>
    ...
    if (step === 'result' && analysis) {
        return (
            <div className="min-h-screen bg-gray-50/50 py-20 px-6 space-y-12">
                <div className="max-w-4xl mx-auto space-y-12">
                    <div className="text-center space-y-4">
                        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 text-blue-700 text-sm font-bold uppercase tracking-wider">
                            <ChartIcon size={18} /> Analisis Akademik & Minat
                        </div>
                        <h2 className="text-4xl md:text-5xl font-black text-gray-900 tracking-tight">Hasil Analisis Lanjutan</h2>
                    </div>

                    {/* Chart Section */}
                    <div className="bg-white p-8 rounded-[3rem] shadow-xl shadow-blue-900/5 border border-gray-100 space-y-8">
                        <div>
                            <h3 className="text-2xl font-bold text-gray-900">Grafik Perkembangan Nilai</h3>
                            <p className="text-gray-400 text-sm">Visualisasi kekuatan akademik anda per bidang setiap semester.</p>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="bg-gray-900 text-white p-8 rounded-[2.5rem] shadow-2xl space-y-6">
                            <div className="space-y-1">
                                <p className="text-blue-400 text-xs font-black uppercase tracking-widest">Akademik Dominan</p>
                            ...
                            <ul className="space-y-4">
                                {ACADEMIC_MAJORS_MAPPING[analysis.dominantField].map((m, i) => (
                                    <li key={i} className="flex items-center gap-4 group">
                                        ...
                                        <span className="text-gray-300 font-bold group-hover:text-white transition-all">{m}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
                        ...
                        {/* Section 2: Combined */}
                        <div className="bg-blue-600 text-white p-8 rounded-[2.5rem] shadow-2xl shadow-blue-600/20 space-y-6">
                            <div className="space-y-1">
                                <p className="text-blue-200 text-xs font-black uppercase tracking-widest">Rekomendasi Final</p>
                                <h3 className="text-2xl font-bold">Jurusan Sesuai Minat & Nilai</h3>
                            </div>
                            <ul className="space-y-4">
                                {ACADEMIC_MAJORS_MAPPING[analysis.finalDomField].map((m, i) => (
                                    <li key={i} className="flex items-center gap-4 group">
                                        <div className="w-8 h-8 rounded-xl bg-white/20 flex items-center justify-center font-bold text-sm">{i + 1}</div>
                    ...
                    <div className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-sm space-y-6">
                        <h3 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                            <ChartIcon className="text-blue-600" /> Analisis Kesimpulan
                        </h3>
                        <div className="p-8 bg-blue-50 rounded-3xl space-y-4">
                            ...
                            <p className="text-blue-900 leading-relaxed text-lg italic">
                                {interestScores ? (
                    ...
                    <div className="flex flex-col sm:flex-row justify-center gap-6 pt-12">
                        <button onClick={() => navigate('/')} className="px-10 py-5 rounded-3xl bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold transition-all flex items-center justify-center gap-2">
